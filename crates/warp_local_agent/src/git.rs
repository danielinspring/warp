...
//!
//! These tools gather repository state via the `crate::git_helpers`
...
async fn resolve_repo_root(path: &Path) -> Option<PathBuf> {
    let output = run_git_command(path, &["rev-parse", "--show-toplevel"])
        .await
        .ok()?;
...
async fn execute_git_status(
    ...
    let head = run_git_command(&repo_root, &["rev-parse", "--short", "HEAD"])
        .await
        .ok()
...
async fn diff_against_base(repo_root: &Path, base: &str) -> anyhow::Result<String> {
    ...
    let end_ref = if run_git_command(repo_root, &["rev-parse", "--verify", &remote_ref])
        .await
        .is_ok()
    {
        remote_ref
    ...
    let diff = run_git_command(repo_root, &["diff", &range]).await?;
...
async fn commit_subjects_since(repo_root: &Path, base: &str) -> Vec<String> {
    ...
    run_git_command(repo_root, &["log", &range, "--format=%s"])
        .await
        .map(|output| {
            output
                .lines()
