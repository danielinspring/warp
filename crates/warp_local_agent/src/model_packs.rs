...
After tools return, summarize from the tool output rather than guessing.",
    ...
    pub fn schema_description_prefix(self, tool_name: &str) -> Option<&'static str> {
        match (self, tool_name) {
            (Self::Qwen, "edit_files") => {
                Some("[Qwen] You MUST call this tool to change files — never shell redirects. ")
            }
            (Self::Qwen, "run_shell_command") => Some(
                "[Qwen] Prefer structured tool_calls; set is_read_only=true for read-only commands. ",
            ),
            (Self::Qwen, "update_todos") => {
                Some("[Qwen] REPLACE the full pending list (omit ids to drop them; do not merge). ")
            }
            (Self::DeepSeek, "edit_files") => Some(
                "[DeepSeek] Call with JSON matching the schema; do not narrate edits without calling. ",
            ),
            (Self::DeepSeek, "run_shell_command") => {
                Some("[DeepSeek] Emit a real tool call with a JSON `command` string. ")
            }
            (Self::DeepSeek, "update_todos") => {
                Some("[DeepSeek] Pass a full `todos` array that replaces pending (not a merge). ")
            }
            (Self::Llama, "edit_files") => {
                Some("[Llama] Use function calling with an `edits` array — not markdown patches. ")
            }
            (Self::Llama, "run_shell_command") => {
                Some("[Llama] Use a tool call, not a fenced shell block. ")
            }
            (Self::Llama, "update_todos") => {
                Some("[Llama] Replace the entire pending todo list via this tool. ")
            }
            (Self::Generic, _) => None,
            (_, _) => None,
        }
...
pub fn apply_schema_tweaks(family: ModelFamily, mut schemas: Vec<ToolSchema>) -> Vec<ToolSchema> {
    ...
    for schema in &mut schemas {
        if let Some(prefix) = family.schema_description_prefix(&schema.name)
            && !schema.description.starts_with(prefix)
        {
