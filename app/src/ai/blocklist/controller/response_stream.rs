...
pub struct ResponseStream {
    ...
    /// The run id the agent visualization keys this stream's agent by, from `StreamInit`.
    agent_viz_run_id: Option<String>,
...
impl ResponseStream {
    ...
    /// Feed the agent visualization from the stream every run produces, so the pane works the same
    /// whether the turn was answered by the local service or by the backend.
    fn publish_agent_viz(&self, event: &api::Event) {
        use crate::ai::agent_viz::event::{AgentVizEvent, publish};

        let run_id = self.agent_viz_run_id.as_deref().unwrap_or_default();
        match event {
            Err(error) => publish(
                run_id,
            ...
            Ok(response_event) => match response_event.r#type.as_ref() {
                Some(warp_multi_agent_api::response_event::Type::Init(init)) => {
                    publish(&init.run_id, AgentVizEvent::TurnStarted)
                }
                Some(warp_multi_agent_api::response_event::Type::Finished(finished)) => {
                    use warp_multi_agent_api::response_event::stream_finished::Reason;
                    let error = match &finished.reason {
                        Some(Reason::InternalError(error)) => Some(error.message.clone()),
                        Some(Reason::Other(_)) => Some("the run ended unexpectedly".to_string()),
                        _ => None,
                    };
                    publish(run_id, AgentVizEvent::Finished { error })
                }
                Some(warp_multi_agent_api::response_event::Type::ClientActions(actions)) => {
                    for event in actions
                        .actions
                        .iter()
                        .filter_map(agent_viz_event_for_action)
                    {
                ...
                if let Some(event_type) = &response_event.r#type {
                    match event_type {
                        warp_multi_agent_api::response_event::Type::Init(init_event) => {
                            ...
                            self.agent_viz_run_id = Some(init_event.run_id.clone());
                        }
...
/// Map a client action onto the agent visualization's vocabulary, ignoring the bookkeeping ones.
fn agent_viz_event_for_action(
    action: &warp_multi_agent_api::ClientAction,
) -> Option<crate::ai::agent_viz::event::AgentVizEvent> {
    use warp_multi_agent_api::client_action::Action;
    use warp_multi_agent_api::message::Message as ProtoMessage;

    use crate::ai::agent_viz::event::AgentVizEvent;

    let messages = match action.action.as_ref()? {
        Action::AddMessagesToTask(add) => add.messages.clone(),
        Action::AppendToMessageContent(append) => append.message.clone().into_iter().collect(),
        _ => return None,
    };
    ...
    messages
        ...
        .find_map(|message| match message.message.as_ref()? {
            ProtoMessage::ToolCall(call) => Some(AgentVizEvent::ToolRequested {
                tool_name: tool_call_label(call),
            }),
            ProtoMessage::AgentOutput(output) => Some(AgentVizEvent::Text {
                preview: output.text.chars().take(120).collect(),
            }),
            _ => None,
        })
}

/// A readable name for a tool call, falling back to the proto variant when the service sent a
/// local-only tool with no wire payload.
fn tool_call_label(call: &warp_multi_agent_api::message::ToolCall) -> String {
